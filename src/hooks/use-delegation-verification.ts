...
export function useDelegationVerification({
  ...
  const verifyDelegation = async (
    ...
    if (isTestMode) {
      ...
      console.log(`[TEST MODE] Mocking delegation verification for delegator: ${delegator}, expected: ${expectedDelegate}`);
      return true;
    }

    try {
      const { target } = await getDelegationStatus(delegator, daoSource, daoIdentifier, tokenAddress, daoChainId);
  ...
  const checkExistingDelegation = async () => {
    ...
    if (isTestMode) {
      ...
      console.log(`[TEST MODE] Mocking delegation check for user: ${userAddress}, dao: ${daoIdentifier}`);
    ...
    try {
      ...
      const { exists, target } = await getDelegationStatus(userAddress, daoSource, daoIdentifier, tokenAddress, daoChainId);
