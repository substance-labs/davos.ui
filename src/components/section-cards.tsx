...
export function SectionCards() {
  ...
  return (
    <div
      className={`${gridContainerClasses} *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs`}
    >
      {Object.entries(daoConfig).map(([key, dao]) => (
        <DaoCard key={key} dao={dao} logo={dao.logo} />
      ))}
    </div>
  );
}
