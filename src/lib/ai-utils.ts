...
export async function fetchOpenAIResponseDirectly(directive: string, proposal: string): Promise<string> {
  ...
  if (isTestMode) {
    ...
    console.log(`[TEST MODE] Mocking OpenAI response for directive: ${directive.substring(0, 50)}...`);
...
export async function fetchOpenAIResponse(directive: string, proposal: string): Promise<string> {
  ...
  if (isTestMode) {
    // Mock response for test environment
    console.log(`[TEST MODE] Mocking Davos AI response for directive: ${directive.substring(0, 50)}...`);
