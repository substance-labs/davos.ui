...
export const SNAPSHOT_SPACE_QUERY = `
...
export async function fetchGraphQL(query: string, variables: Record<string, any> = {}) {    
  ...
  if (isTestMode) {
    ...
    if (query.includes('query Space')) {
      const spaceId = variables.id || variables.space;
      try {
        ...
        if (proposals) {
          return {
            space: {
              ...
              votesCount: proposals.reduce((sum: number, p: any) => sum + (p.scores_total || 0), 0),
              followersCount: 1000
            }
          };
        }
...
export async function fetchSpace(spaceId: string) {
  const result = await fetchGraphQL(SNAPSHOT_SPACE_QUERY, { id: spaceId });
  return result.space;
}
